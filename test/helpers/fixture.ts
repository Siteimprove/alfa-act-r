...
export function fixture(
  ...
  rule: Maybe<Rule<Page, T, Question.Metadata, S>>,
  options?: Fixture.Options,
) => Promise<void> {
  return async (t, rule, options = {}) => {
    for (const theRule of Maybe.toOption(rule)) {
  ...
  switch (result) {
    case "ok": // Alfa and ACT-R perfectly agree
      ...
      if (strict && (skip || manual || lax)) {
        ...
        t.log("Test", test);
        t.fail(
          `${test.id} produces correct result, but is marked as ${skip ? "skipped" : manual ? "manual" : "lax"}`,
        );
      } else {
        t.pass(test.id);
